...
int main(int argc, char *argv[])
{
  ...
  //1. 초기화& 플레이어 이름 결정
  board_initboard();
  //1-2. PLAYER INIT.-초기화는 한번실행하고 땡이라서 함수화가 필수는 아님.
  for(i=0;i<N_PLAYER;i++)
  {
       player_position[i] = 0;
       player_coin[i] = 0;
       ...
       scanf("%s", player_name[i]); //자체가 포인터라 &필요없음.           
  ...
  do{
       ...
       if (player_status[turn] != PLAYERSTATUS_LIVE)//죽거나 끝났을 경우일때 참이어야함. 
       {
            turn = (turn +1)%N_PLAYER; 
            continue;//어떤 조건 만족시에 아래거실행안하고 바로위로다시올라감. 
       }
       //2-1. 플레이어 상태 출력
       ...
       scanf("%d", &c);//while문 막 돌아가서 주체가안됨. 그래서 넣은 것임. 
